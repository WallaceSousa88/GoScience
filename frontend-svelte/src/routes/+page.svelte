<script lang="ts">
  ...
  function abrirModal(materia: string) {
    modalAberto = materia
  }
...
<div class="flex flex-col flex-1 w-full px-4 py-8">
  <div class="flex-1 flex items-center justify-center">
    <div class="text-center">
      <h1 class="text-4xl font-bold text-black mb-2">Bem-vindo!</h1>
      <p class="text-lg text-black max-w-3xl">
        <br>Explore conceitos de Física, Química e Matemática com simulações interativas.<br>
        Clique nos cards para começar sua jornada científica.
      </p>
    </div>
  </div>

  <section class="grid grid-cols-1 sm:grid-cols-3 gap-8 w-full max-w-5xl self-center">
  ...
  <div class="flex-1 flex items-center justify-center">
    <div class="text-center max-w-2xl">
      <blockquote class="italic text-black">
        “A imaginação frequentemente nos leva a mundos que nunca existiram.<br>
        Mas sem ela, não vamos a lugar nenhum.”
      </blockquote>
      <span class="block mt-4 font-semibold text-black">— Carl Sagan</span>
    </div>
  </div>
  ...
  {#if modalAberto}
    <div
      ...
      on:click={(e) => e.currentTarget === e.target && fecharModal()}
      on:keydown={(e) => (e.key === 'Enter' || e.key === ' ') && fecharModal()}
    >
      <div
